...
function resolveClaudeBinary(binPath: string): { cmd: string; args: string[] } | null {
  ...
  if (resolved.endsWith(".js") || resolved.endsWith(".cjs") || resolved.endsWith(".mjs")) {
    console.error(`[ClaudeRemote] Using JS entry: ${resolved}`)
  ...
  if (binPath.endsWith(".js") || binPath.endsWith(".cjs") || binPath.endsWith(".mjs")) {
    console.error(`[ClaudeRemote] Using JS entry: ${binPath}`)
    return { cmd: process.execPath, args: [binPath] }
  }

  // Native binary — use directly
  if (process.platform !== "win32") {
    console.error(`[ClaudeRemote] Using native binary: ${resolved}`)
  ...
  if (fs.existsSync(cliJs)) {
    console.error(`[ClaudeRemote] Resolved to cli.js`)
  ...
  if (platformInfo) {
    ...
    if (fs.existsSync(nativeBin) && fs.statSync(nativeBin).size > 1000) {
      console.error(`[ClaudeRemote] Using native binary: ${nativeBin}`)
  ...
  if (fs.existsSync(wrapper)) {
    console.error(`[ClaudeRemote] Using cli-wrapper.cjs fallback`)
...
function startPty() {
  ...
  process.stdin.on("data", (data: Buffer) => {
    ...
    if (planPromptActive && (text.includes("\r") || text.includes("\n"))) {
      ...
      send({
        type: "opencode_event",
        event: { type: "plan.replied", properties: { sessionID: sessionId } },
      })
    }
    ...
    if (pendingPerms.size > 0) {
      if (text.includes("y") || text.includes("n")) {
        for (const [, entry] of pendingPerms) {
          ...
          send({
            type: "opencode_event",
            event: { type: "permission.replied", properties: { sessionID: sessionId } },
          })
        }
        pendingPerms.clear()
      }
...
function handlePreToolUse(data: Record<string, any>) {
  ...
  let checkAttempt = 0
  const MAX_CHECKS = 14 // 14 × 350ms ≈ 4.9s
 ...
 */
function extractResponseText(): { text: string; hasPendingTools: boolean } {
  if (!transcriptPath || !lastUserPrompt) return { text: "", hasPendingTools: false }
  try {
    if (!fs.existsSync(transcriptPath)) return { text: "", hasPendingTools: false }
    ...
    if (promptIdx < 0) return { text: "", hasPendingTools: false }
      ...
      try {
        ...
        for (const block of contentArr) {
          if (block.type === "text" && block.text?.trim()) {
            ...
            const inputLimit = (toolName === "Edit" || toolName === "MultiEdit" || toolName === "Write") ? 5000 : 500
    ...
    return { text: "", hasPendingTools: false }
  }
}
      ...
      for (let i = rawLines.length - 1; i >= Math.max(0, rawLines.length - 5); i--) {
        ...
        try {
          ...
          if (msg.type === "user") {
            ...
            if (Array.isArray(contentArr)) {
              for (const block of contentArr) {
                if (block.type === "text" && block.text?.includes("[Request interrupted by user for tool use]")) {
                  interrupted = true
                  break
                }
              }
      ...
      try {
        ...
        if (msg.type === "user") {
          ...
          if (typeof contentArr === "string") {
            ...
            if (info) {
              const partType = info.isCommand ? "command-output" : "text"
              userMessages.push({
                idx: msgIdx++,
            ...
            if (block.type === "text" && block.text?.trim()) {
              ...
              if (toolName === "AskUserQuestion" || toolName === "AskQuestion") {
                ...
                while ((am = ansRegex.exec(rawOutput)) !== null) {
                  if (qi < qItems.length) qAnswers[qi] = [am[2]]
                  qi++
                }
...
function finalizeResponse() {
  ...
  // If there are tool_use blocks without tool_result yet, keep polling
  if (extractResult.hasPendingTools) {
  ...
  // Get full text for caching (may include history beyond current response)
  const transcriptText = extractTranscriptText()
  const stopText = accumulatedText
  const fullText = stopText || finalResponseText || transcriptText
  ...
  const qResult = lastQuestionResult
...
function sendComplete(msgId: string) {
  send({
    type: "opencode_event",
    event: {
      type: "message.complete",
      properties: { messageID: msgId, sessionID: sessionId },
    },
  })
  send({
    type: "opencode_event",
...
function handlePostToolUse(data: Record<string, any>) {
  ...
  if ((toolName === "AskUserQuestion" || toolName === "AskQuestion") && pendingQuestions.length > 0) {
    ...
    send({
      type: "opencode_event",
      event: { type: "question.replied", properties: { sessionID: sessionId } },
    })
  }
  ...
  try {
    ...
    // ── Abort: Ctrl+C ──
      ...
      if (ptyProc) {
        ...
        if (hadPermission) {
          send({ type: "opencode_event", event: { type: "permission.replied", properties: { sessionID: sessionId } } })
        }
        if (hadQuestion) {
          send({ type: "opencode_event", event: { type: "question.replied", properties: { sessionID: sessionId } } })
        }
        if (hadPlan) {
          send({ type: "opencode_event", event: { type: "plan.replied", properties: { sessionID: sessionId } } })
        }
        send({ type: "opencode_event", event: { type: "session.idle", properties: { sessionID: sessionId } } })
        result = { ok: true }
      } else {
    ...
    // We navigate with arrow keys (\x1b[B = down) and confirm with Enter (\r)
      ...
      if (ptyProc) {
        if (choice === "manual") {
          // Option 2: down arrow once + Enter
          ptyProc.write("\x1b[B\r")
        } else if (choice === "feedback") {
          // Option 3: down arrow twice + Enter, then type feedback
          ptyProc.write("\x1b[B\x1b[B\r")
    ...
    // ── Question reply: approve permission + inject answer into PTY ──
      ...
      const isReject = path?.endsWith("/reject") || !!body?.reject
      const qData = pendingQuestions.shift()
      const questions = qData?.questions || []

      if (isReject) {
        // Log PTY buffer to diagnose what's showing in terminal
        const recentPty = ptyBuffer.slice(Math.max(0, ptyBuffer.length - 600))
        // Check if permission dialog is still showing (not auto-approved)
        ...
        if (hasPermDialog) {
          // Permission dialog is showing: approve it first, then reject question
          setTimeout(() => {
            if (!ptyProc) return
            ptyProc.write("\r")
          }, 0)
          ...
          // Question UI is showing: press Escape to cancel (UI shows "Esc to cancel")
          setTimeout(() => {
            if (!ptyProc) return
            ptyProc.write("\x1b")
          }, 500)
        ...
        send({
          type: "opencode_event",
          event: { type: "question.rejected", properties: { sessionID: sessionId } },
        })
        send({ type: "opencode_event", event: { type: "session.idle", properties: { sessionID: sessionId } } })
      } else {
        const answers: string[][] = body?.answers || []
        if (answers.length > 0 && ptyProc) {
          // Process all questions sequentially with delays between each
          const totalQuestions = Math.min(answers.length, questions.length)
          for (let qi = 0; qi < totalQuestions; qi++) {
            const ans = answers[qi]
            const q = questions[qi]

            const baseDelay = qi * 2000 + 500

            // Check if permission dialog is showing or already auto-approved
            const recentPty = ptyBuffer.slice(Math.max(0, ptyBuffer.length - 600))
            const hasPermDialog = /Do you want|Allow.*to|to proceed/i.test(recentPty)

            // Step 1: only send Enter if permission dialog is actually showing
            if (hasPermDialog) {
              setTimeout(() => {
                if (!ptyProc) return
                ptyProc.write("\r")
            ...
            // Step 2: navigate to the selected option(s)
            // Always use baseDelay + 1200 timing to ensure question UI is ready
            if (q?.options?.length && ans.length > 0) {
              const isMultiSelect = q.multiSelect === true
              if (isMultiSelect) {
                // Multi-select: Enter to toggle each option, then Tab to Submit + Enter to confirm
                const selectedIndices = ans.map(label => {
                  const idx = q.options!.findIndex((o: any) => (o.label || o) === label)
                  return { label, idx }
                }).filter(x => x.idx >= 0).sort((a, b) => a.idx - b.idx)

                let currentPos = 0
                selectedIndices.forEach(({ label, idx }, si) => {
                  const steps = idx - currentPos
                  currentPos = idx
                  setTimeout(() => {
                    if (!ptyProc) return
                    if (steps > 0) ptyProc.write("\x1b[B".repeat(steps))
                    ptyProc.write("\r")
                  }, baseDelay + 1200 + si * 800)
                })
                // Submit: log PTY snapshot, then try Tab + Enter
                setTimeout(() => {
                  if (!ptyProc) return
                  const recentPty2 = ptyBuffer.slice(Math.max(0, ptyBuffer.length - 800))
                  ptyProc.write("\t")
                }, baseDelay + 1200 + selectedIndices.length * 800 + 500)
                setTimeout(() => {
                  if (!ptyProc) return
                  ptyProc.write("\r")
                }, baseDelay + 1200 + selectedIndices.length * 800 + 1200)
              } else {
                // Single-select: navigate to option and press Enter
                const selectedLabel = ans[0]
                const optIdx = q.options!.findIndex((o: any) =>
                  (o.label || o) === selectedLabel
                )
                setTimeout(() => {
                  if (!ptyProc) return
                  if (optIdx >= 0) {
                    const navKeys = "\x1b[B".repeat(optIdx) + "\r"
                    ptyProc.write(navKeys)
                  } else {
                    ptyProc.write(selectedLabel + "\r")
                  }
                }, baseDelay + 1200)
              }
            } else {
        ...
        // Store question result for real-time structured part event in finalizeResponse
        if (questions.length > 0 && answers.length > 0) {
          lastQuestionResult = { questionItems: questions, questionAnswers: answers }
        }
      }
      result = { ok: true }
    }
...
function handleExecuteCommand(body: Record<string, any> | undefined, reqId: string = ""): any {
  ...
  switch (cmd) {
    ...
    case "cost": {
      if (!ptyProc) return { error: "PTY not running" }
      const costBufStart = ptyBuffer.length
      ...
      const costTimer = setInterval(() => {
        costCheckCount++
        const newOutput = ptyBuffer.slice(costBufStart)
        // Strip ANSI codes and control chars
        // Preserve cursor row changes as newlines before stripping CSI sequences
        let text = newOutput
          .replace(/\x1b\[(\d+);(\d+)[Hf]/g, (_, row, col) => {
            // Cursor position: if row > 1, it moved to a new line
            return parseInt(row) > 0 ? "\n" : ""
          })
          .replace(/\x1b\[\d*[AB]/g, m => {
            // Cursor up/down: always a line change
            return "\n"
          })
          .replace(/\x1b\[[0-9;?]*[a-zA-Z]/g, "")
          .replace(/\x1b\][^\x07]*\x07/g, "")
          ...
          .replace(/\r\n/g, "\n").replace(/\r/g, "\n")
        // Remove prompt line content but keep ⎿-prefixed cost data on same line
        text = text.replace(/^[^]*?[❯›>]\s*\/cost\s*/, "")
        // ⎿ separates prompt from output on same line — split there
        text = text.replace(/[⎿└┘┌╭╮╰╯│├┤][\s─━]*?Total/g, "\nTotal")
        text = text.replace(/[⎿└┘┌╭╮╰╯│├┤][\s─━]*?Usage/g, "\nUsage")
        // Use splitTerminalOutput to handle cursor-positioned merged text
        text = splitTerminalOutput(text)
        // Normalize: ensure field prefixes are on separate lines
        text = text.replace(/\s*(Total\s+(?:cost|duration|code))/gi, "\n$1")
        text = text.replace(/\s*(Usage\s+by\s+model)/gi, "\n$1")
        // Fix missing spaces in field names
        text = text.replace(/Totalcost/gi, "Total cost")
        text = text.replace(/Totalcode/gi, "Total code")
        // Fix joined values: "289linesadded" → "289 lines added"
        text = text.replace(/(\d+)(lines?\s*added)/gi, "$1 $2")
        text = text.replace(/,(\d+)(lines?\s*removed)/gi, ", $1 $2")
        // Split model sub-items to own lines with indent
        text = text.replace(/\s+([a-z][\w.-]*\d[\w.-]*:\s+\d)/gi, "\n  $1")
        // Filter out junk lines
        const lines = text.split("\n")
          .map(l => l.replace(/^[⎿└┘┌─│├┤╭╮╰╯]+\s*/, "").trim())
          .filter(l => l && !/^[❯›>⏵▸▶╭╮╰╯│├┤⎿└┘┌]/.test(l)
            && !/accept edits|shift\+tab|to cycle/i.test(l)
            && !/^[─━\-═]+$/.test(l))
        const hasContent = lines.some(l => /cost|duration|usage|model/i.test(l))
        if (hasContent) {
          const resultText = lines.join("\n")
          if (resultText === costPrevText) {
            costStable++
          } else {
            costStable = 0
            costPrevText = resultText
          }
          if (costStable >= 2 || costCheckCount > 15) {
            clearInterval(costTimer)
            lines.forEach((l, i) => log(`[cost] [${i}] ${l}`))
            send({ type: "api_response", reqId: costReqId, status: 200, body: { text: resultText } })
          }
        }
      }, 400)
      return "__DEFERRED__"
